...
def extract_feature_maps():
    ...
    # 학습과 동일하게 in_channels=2 (Y + grid)로 초기화
    model = Autoenc(in_channels=2).to(device)
    ...
    for fname in sorted(f for f in os.listdir(in_dir) if f.endswith(".yuv")):
        ...
        grid_name = str(w) + "x" + str(h) + ".yuv"
        grid_path = os.path.join(grid_dir, grid_name)

        y_tensor    = torch.from_numpy(y).unsqueeze(0).unsqueeze(0).to(device)  # [1,1,H,W]
        grid_tensor = load_y_frame_400_10bit_le(grid_path, w, h)
        grid_tensor = torch.from_numpy(grid_tensor).unsqueeze(0).unsqueeze(0).to(device)
        ...
        with torch.no_grad():
            ...
            input_x = torch.cat([y_tensor, grid_tensor], dim=1)                 # [1,2,H,W]
            ...
            out2 = model.ResB3(model.ResB2(model.ResB1(branch2)))
